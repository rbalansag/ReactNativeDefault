...
export default function Index(props: iProps) {
   ...
   return (
      <View style={{justifyContent: 'center', flex: 1, alignItems: 'center'}}>
         <Text style={{fontSize: 50}}>Your drivers license</Text>
         <TouchableOpacity
             onPress={async () => {
               const [{status}] = await Promise.allSettled([
               // preload group first before navigation *
               preload().group('SIGNUP'),
               preload().component(navigationStrings.SIGNUP_PASSWORD),
            ]);
               if (status === FULFILLED) {
               props.navigation.push(navigationStrings.SIGNUP_PASSWORD);
               }
            }}>
